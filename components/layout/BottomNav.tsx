...
export function BottomNav() {
  ...
  return (
    <motion.nav
      ...
      className="fixed left-2 md:left-6 top-1/2 -translate-y-1/2 z-[100]"
    >
      <div className="relative flex flex-col items-center gap-2 md:gap-3 p-1.5 md:p-3 bg-[#0d0d0d]/95 backdrop-blur-2xl border border-white/10 rounded-xl md:rounded-2xl shadow-2xl shadow-black/60">
        {/* Glow effect */}
              ...
              <Link href="/perfil" className="block group">
                <div className="relative">
                  ...
                  <Image
                    ...
                    width={56}
                    height={56}
                    className="relative rounded-full border-2 border-white/20 group-hover:border-white/40 transition-all object-cover shadow-lg w-10 h-10 md:w-14 md:h-14"
                  />
                </div>
                {/* Online indicator */}
                <span className="absolute bottom-0 right-0 w-2 h-2 md:w-3 md:h-3 bg-emerald-500 border-2 border-[#0d0d0d] rounded-full" />
              </Link>
            </motion.div>
          )}
        </AnimatePresence>
        ...
        <AnimatePresence>
          {!isHome && (
            <motion.div
              ...
              className="w-6 md:w-8 h-px bg-white/10 mb-0.5 md:mb-1"
            />
          )}
        </AnimatePresence>
        ...
        {NAV_ITEMS.map((item) => {
          ...
          return (
            ...
            >
              <motion.div
                className={`
                  relative flex flex-col items-center justify-center w-10 md:w-16 py-1.5 md:py-3 px-1 md:px-2 rounded-lg md:rounded-xl
              ...
              >
                ...
                {/* Label below icon */}
                <span className={`relative z-10 text-[7px] md:text-[9px] font-medium mt-0.5 md:mt-1 tracking-wide ${isActive ? 'text-white' : 'text-white/50'}`}>
                  {item.label}
                </span>
              </motion.div>
